...
# Review the list of TLDs above (manually or KQL), then come back here to add/remove exclusions as necessary. NO RESPONSIBILITY IS TAKEN IF YOU CAUSE BUSINESS IMPACT.
...
foreach ($line in $BlockList.Split([Environment]::NewLine)) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($trimmedLine) -and -not $trimmedLine.StartsWith("#") -and $exclusion -notcontains $trimmedLine) {
        ...
        if ($trimmedLine -match '^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$') {
            # Format for Senders & Domains blocking: *.<tld>
            # This blocks the TLD itself, all subdomains under it, and any sender email address in it
            $domainPattern = "*.$trimmedLine".ToLowerInvariant()
            Write-Host "Blocking sender domain pattern: $domainPattern"
            New-TenantAllowBlockListItems -ListType Sender -Block -Entries $domainPattern -NoExpiration -Notes "Blocked TLD per https://github.com/jkerai1/TLD-TABL-Block/tree/main"
        } else {
